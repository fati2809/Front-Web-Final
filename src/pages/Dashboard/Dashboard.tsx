            ...
            {activeData.length === 0 ? (
              ...
              <svg className="chart-svg" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
                ...
                {[0, 0.25, 0.5, 0.75, 1].map((t, i) => (
                  <line key={i} x1={padX} y1={padY + t * (height - padY * 2)} x2={width - padX} y2={padY + t * (height - padY * 2)} stroke="#e2e8f0" strokeWidth="1" strokeDasharray={t === 0 ? "0" : "5,5"} />
                ))}

                {[0, 0.25, 0.5, 0.75, 1].map((t, i) => (
                  <text key={i} x={padX - 8} y={padY + (1 - t) * (height - padY * 2) + 4} textAnchor="end" fontSize="10" fill="#9ca3af">
                ...
                {pathD && (
                  <path d={pathD} fill="none" stroke="#9f7aea" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                )}
