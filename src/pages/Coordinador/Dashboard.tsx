...
function CoordinadorDashboard() {
  ...
  try {
    ...
    const rect      = (x: number, yy: number, w: number, h: number, style: "F"|"S"|"FD" = "F") => doc.rect(x, yy, w, h, style);
    const checkPage = (needed: number) => { if (y + needed > pageH - 10) { doc.addPage(); y = margin; } };

    // Encabezado
    setFill(PURPLE); rect(0, 0, pageW, 28);
    setFont(18, "bold", WHITE);
    doc.text("Reporte — Coordinador", margin, 16);
    setFont(9, "normal", [200, 190, 240] as [number, number, number]);
    doc.text(`Generado el ${new Date().toLocaleString("es-MX", { dateStyle: "full", timeStyle: "short" })}`, margin, 23);
    y = 36;

    // Métricas
    setFont(11, "bold", TEXT_DK); doc.text("Resumen general", margin, y); y += 5;
    ...
    [
      ...
      setFill(GRAY_BG); setDraw(GRAY_LN); rect(cx, y, cardW, 22, "FD");
      setFill(card.color); rect(cx, y, 3, 22);
      setFont(18, "bold", card.color); doc.text(card.value, cx + 10, y + 14);
      setFont(8, "normal", TEXT_MD);   doc.text(card.label,  cx + 10, y + 20);
    });
    y += 30;
    ...
    if (activeData.length > 0) {
      ...
      doc.text(`Gráfica de eventos — período: ${periodo}`, margin, y); y += 4;
      const gW = contentW, gH = 52, gPX = 12, gPY = 6;
      const gX0 = margin + gPX, gX1 = margin + gW - gPX;
      const gY0 = y + gPY,      gY1 = y + gH - gPY;
      const gMx = Math.max(...activeData.map(d => d.value), 1);

      setFill(GRAY_BG); setDraw(GRAY_LN); rect(margin, y, gW, gH, "FD");
      doc.setLineWidth(0.2);
      for (let t = 0; t <= 4; t++) {
        const ly = gY0 + (t / 4) * (gY1 - gY0);
        setDraw([210, 215, 225] as [number, number, number]); doc.line(gX0, ly, gX1, ly);
        setFont(5, "normal", TEXT_MD); doc.text(String(Math.round(gMx * (1 - t / 4))), margin + 1, ly + 1.5);
      }
      const pts = activeData.map((d, i) => ({
        x: activeData.length === 1 ? (gX0 + gX1) / 2 : gX0 + (i / (activeData.length - 1)) * (gX1 - gX0),
        y: gY0 + (1 - d.value / gMx) * (gY1 - gY0),
      }));
      doc.setLineWidth(1); setDraw(PURPLE);
      for (let i = 1; i < pts.length; i++) doc.line(pts[i-1].x, pts[i-1].y, pts[i].x, pts[i].y);
      pts.forEach((p, i) => {
        setFill(WHITE); setDraw(PURPLE); doc.setLineWidth(0.8); doc.circle(p.x, p.y, 1.2, "FD");
        setFont(5, "normal", TEXT_MD); doc.text(activeData[i].label, p.x, gY1 + 4, { align: "center" });
        if (activeData[i].value > 0) { setFont(5, "bold", PURPLE); doc.text(String(activeData[i].value), p.x, p.y - 2, { align: "center" }); }
      });
      y += gH + 8;
    }
    ...
    // Tabla Eventos
    checkPage(20);
    setFont(11, "bold", TEXT_DK); doc.text("Detalle de eventos", margin, y); y += 4;
    const colsEv = [
      { header: "Nombre",   w: 55 }, { header: "Edificio", w: 38 },
      { header: "Fecha",    w: 42 }, { header: "Profesor",  w: 38 }, { header: "Status", w: 19 },
    ];
    const rowH = 7;

    const drawHeader = (cols: typeof colsEv, yy: number) => {
      let cx = margin; setFill(PURPLE); rect(margin, yy, contentW, rowH);
      setFont(7, "bold", WHITE);
      cols.forEach(col => { doc.text(col.header, cx + 2, yy + 5); cx += col.w; });
      return yy + rowH;
    };
    const drawRow = (cols: typeof colsEv, values: string[], yy: number, shade: boolean) => {
      setFill(shade ? GRAY_BG : WHITE); setDraw(GRAY_LN); doc.setLineWidth(0.1);
      rect(margin, yy, contentW, rowH, "FD");
      let cx = margin; setFont(7, "normal", TEXT_DK);
      cols.forEach((col, i) => {
        const val = String(values[i] ?? "");
        if (col.header === "Status") {
          setFill(val === "Activo" ? GREEN : RED); rect(cx + 1.5, yy + 1.8, col.w - 3, 3.5, "F");
          setFont(6, "bold", WHITE); doc.text(val, cx + col.w / 2, yy + 4.3, { align: "center" });
          setFont(7, "normal", TEXT_DK);
        } else {
          doc.text(doc.splitTextToSize(val, col.w - 3)[0] ?? val, cx + 2, yy + 5);
        }
        cx += col.w;
      });
      return yy + rowH;
    ...
    y = drawHeader(colsEv, y);
    (data.eventos as any[]).slice(0, 30).forEach((ev, idx) => {
      checkPage(rowH + 2);
      if (y === margin) y = drawHeader(colsEv, y);
      y = drawRow(colsEv,
        [ev.name_event, ev.name_building ?? "—",
         ev.timedate_event ? new Date(ev.timedate_event).toLocaleDateString("es-MX") : "—",
         ev.nombre_profe ?? "—", ev.status_event === 1 ? "Activo" : "Inactivo"],
        y, idx % 2 === 0);
    });
    y += 8;
    ...
    // Tabla Usuarios
    checkPage(20);
    setFont(11, "bold", TEXT_DK); doc.text("Detalle de usuarios", margin, y); y += 4;
    const colsUs = [
      { header: "Nombre",    w: 60 }, { header: "Email",     w: 68 },
      { header: "Matrícula", w: 24 }, { header: "Rol",       w: 30 },
    ];
    y = drawHeader(colsUs, y);
    (data.usuarios as any[]).slice(0, 30).forEach((u, idx) => {
      checkPage(rowH + 2);
      if (y === margin) y = drawHeader(colsUs, y);
      y = drawRow(colsUs, [u.name_user, u.email_user, String(u.matricula_user), u.rol], y, idx % 2 === 0);
    });

    // Pie de página
    const totalPages = (doc as any).internal.getNumberOfPages();
    for (let pg = 1; pg <= totalPages; pg++) {
      doc.setPage(pg); setFill(PURPLE); rect(0, pageH - 8, pageW, 8);
      setFont(7, "normal", [200, 190, 240] as [number, number, number]);
  ...
  // Gráfica
  const activeData = graficaData.map(d => ({ label: d.label, value: d.eventos ?? 0 }));
  ...
  const width = 800; const height = 250; const padX = 45; const padY = 20;
  const points = activeData.map((d, i) => ({
    x: activeData.length === 1 ? width / 2 : padX + (i / (activeData.length - 1)) * (width - padX * 2),
    ...
    <div className="dashboard-container">
      <aside className="sidebar">
        <nav className="sidebar-nav">
          <button className="nav-item active" onClick={() => navigate("/coordinador/dashboard")}>
          ...
          <button className="nav-item" onClick={() => navigate("/coordinador/eventos")}>
            <span className="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <main className="main-content">
        ...
        <div className="metrics-container">
          <div className="metric-card">
            ...
            <div className="metric-content"><div className="metric-value">{stats.total_usuarios.toLocaleString()}</div></div>
          </div>
          <div className="metric-card">
            <div className="metric-header"><h3 className="metric-title">Total Eventos</h3></div>
            <div className="metric-content"><div className="metric-value">{stats.total_eventos.toLocaleString()}</div></div>
          </div>
        </div>
        ...
        <div className="chart-container">
          <div className="chart-header">
            ...
            <div className="chart-controls">
              <select className="chart-control-btn" value={periodo}
              ...
              <div style={{ position: "relative" }}>
                <button className="chart-control-btn" onClick={() => setShowExportMenu(!showExportMenu)} title="Descargar reporte">
                  {loadingReporte ? "..." : (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                {showExportMenu && (
                  <div style={{ position: "absolute", right: 0, top: "110%", zIndex: 100, background: "#fff", border: "1px solid #e5e7eb", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", overflow: "hidden", minWidth: "160px" }}>
          ...
          <div className="chart-area">
            {activeData.length === 0 ? (
              ...
              <svg className="chart-svg" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
                ...
                {yTicks.map((tick, i) => (
                  <line key={i} x1={padX} y1={padY + (1 - tick / maxValue) * (height - padY * 2)}
                    x2={width - padX} y2={padY + (1 - tick / maxValue) * (height - padY * 2)}
                    stroke="#e2e8f0" strokeWidth="1" strokeDasharray={i === 0 ? "0" : "5,5"} />
                ))}
                {yTicks.map((tick, i) => (
                  <text key={i} x={padX - 8} y={padY + (1 - tick / maxValue) * (height - padY * 2) + 4}
                    textAnchor="end" fontSize="10" fill="#9ca3af">{tick}</text>
                ))}
                {areaD && <path d={areaD} fill="url(#areaGradient)" />}
                {pathD && <path d={pathD} fill="none" stroke="#9f7aea" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />}
                {points.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r="4" fill="#fff" stroke="#9f7aea" strokeWidth="2" />)}
                {points.map((p, i) => <text key={i} x={p.x} y={height - 2} textAnchor="middle" fontSize="10" fill="#9ca3af">{activeData[i].label}</text>)}
              </svg>
            )}
          </div>
        </div>
